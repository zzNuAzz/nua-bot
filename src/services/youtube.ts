...
import { youtubeVideoRegex, youtubePlaylistRegex } from '@/constants/regex';
...
export class YoutubeVideoService {
	public static async getVideoDetails(content: string, index? : number): Promise<Song> {
	...
	private static async searchVideo(
		keyword: string,
		index?: number
	): Promise<string> {
		...
		const item = (
			index && index < filteredRes.length
				? filteredRes[index]
				: filteredRes[0]
		) as Video;
		return item.id;
